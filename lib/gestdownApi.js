...
async function searchShow(query, retries = 3) {
    try {
        ...
        const searchUrl = `${API_URL}/shows/search/${encodeURIComponent(cleanedQuery)}`;
...
async function getSubtitles(showUniqueId, season, episode, language, retries = 3) {
    try {
        ...
        if (response.data && Array.isArray(response.data)) {
            console.log(`Sous-titres récupérés pour ${showUniqueId} S${season}E${episode} en ${normalizedLang}: ${response.data.length} résultats`);
